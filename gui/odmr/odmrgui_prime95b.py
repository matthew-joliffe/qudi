...
class ODMRMainWindow(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        ui_file = os.path.join(this_dir, 'ui_odmrgui_prime95b.ui')
...
class ODMRGui(GUIBase):
    ...
    def on_activate(self):
        ...
        *.ui file and configures the event handling between the modules.
        ...
        # Create a QSettings object for the mainwindow and store the actual GUI
        # layout
        ...
        # Adjust range of scientific spinboxes above what is possible in Qt
        # Designer
        ...
        self._mw.odmr_matrix_PlotWidget.setLabel(
            axis='left', text='Matrix Lines', units='#')
        self._mw.odmr_matrix_PlotWidget.setLabel(
            axis='bottom', text='Frequency', units='Hz')
        ...
        ##
        self._mw.exp_time_SpinBox.setValue(self._odmr_logic.exp_time)
        ...
        ##
        self._mw.do_pixel_fit_PushButton.clicked.connect(self.do_pixel_fit)

        # Control/values-changed signals to logic
        self.sigCwMwOn.connect(
            self._odmr_logic.mw_cw_on,
            QtCore.Qt.QueuedConnection)
        self.sigMwOff.connect(
            self._odmr_logic.mw_off,
            QtCore.Qt.QueuedConnection)
        self.sigClearData.connect(
            self._odmr_logic.clear_odmr_data,
            QtCore.Qt.QueuedConnection)
        self.sigStartOdmrScan.connect(
            self._odmr_logic.start_odmr_scan,
            QtCore.Qt.QueuedConnection)
        self.sigStopOdmrScan.connect(
        ...
        self.sigMwSweepParamsChanged.connect(
            self._odmr_logic.set_sweep_parameters,
            QtCore.Qt.QueuedConnection)
        self.sigRuntimeChanged.connect(
            self._odmr_logic.set_runtime,
            QtCore.Qt.QueuedConnection)
        ##
        self.sigExpTimeChanged.connect(
            self._odmr_logic.set_exp_time,
            QtCore.Qt.QueuedConnection)
        ...
        # QtCore.Qt.QueuedConnection)
        # Exposure time should control clock frequency. Calc done in Logic
        # self.sigExpTimeChanged.connect(self._odmr_logic.set_clock_frequency,
        #                                  QtCore.Qt.QueuedConnection)
        self.sigOversamplingChanged.connect(
            self._odmr_logic.set_oversampling,
            QtCore.Qt.QueuedConnection)
        self.sigLockInChanged.connect(
            self._odmr_logic.set_lock_in,
            QtCore.Qt.QueuedConnection)
    ...
    def on_deactivate(self):
        ...
        ##
        self.sigExpTimeChanged.disconnect()
    ...
    def update_settings(self):
        ...
        ##
        clock_frequency = 1. / \
            ((self._mw.exp_time_SpinBox.value() / 1000.) + 0.2)
